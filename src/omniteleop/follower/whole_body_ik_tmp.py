...
HW-WORKAROUND(right-arm-frozen) -- TEMPORARY FORK of ``whole_body_ik.py``. Delete this file,
``wbik_tmp.yaml``, and ``scripts/wbc_vr_robot_tmp.py`` once the right arm can be used again;
the canonical ``whole_body_ik.py`` is untouched and still serves every other caller (sim
follower, policy rollouts, leader alignment, joystick pipeline). Differences from canonical,
all tagged ``HW-WORKAROUND(right-arm-frozen)``:
  1. ``DEFAULT_CONFIG_PATH`` -> ``wbik_tmp.yaml`` (which adds the ``dead_joints:`` key);
  2. ``WBCConfig.dead_joints`` + ``_dead_joint_pin_A`` built in ``__init__``;
  3. ``_add_dead_joint_equalities`` (``dq = 0`` rows) called from ``solve``;
  4. ``_apply_dead_joint_mask`` applied pre-integration;
  5. every configured pin is forced to its ``nominal_posture`` value on reset/measured
     updates, so the model cannot anchor a frozen joint away from nominal;
  6. an EE FrameTask whose whole chain to the planar root is pinned (``_frame_rigid_to_base``)
     is left OUT of the QP -- it could no longer pose its link, only anchor the base against
     the head world-position term (see ``_build_tasks_and_limits``).
With ``dead_joints: []`` this file is behaviorally identical to the canonical module.
To re-sync after canonical changes: re-copy ``whole_body_ik.py`` and re-apply those six.
...
# HW-WORKAROUND(right-arm-frozen): this TEMPORARY fork reads wbik_tmp.yaml, not the canonical
# wbik.yaml, so `WBCConfig()` here picks up `dead_joints:` while every other importer of the
# real whole_body_ik.py keeps loading the untouched canonical config. Delete this fork (and
# wbik_tmp.yaml) when the right arm can be used again.
DEFAULT_CONFIG_PATH = Path(__file__).with_name("wbik_tmp.yaml")
...
class WBCConfig:
    ...
    # HW-WORKAROUND(right-arm-frozen): TEMPORARY. Joints held at their configured nominal
    # posture and pinned out of the QP by URDF joint name -- one hard `dq = 0` equality row
    # each, every tick, regardless of head_mode. Unlike head_ik_pinned_joints these may be
    # ANY joint of the reduced model. The legacy ``dead_joints`` name is retained so the
    # temporary sim/real followers share one config. Empty (canonical behavior) pins none.
    dead_joints: Sequence[str] = tuple(_DEFAULTS.get("dead_joints", ()))
...
class VegaWholeBodyIK:
    ...
    def _build_model(self) -> None:
        ...
        # HW-WORKAROUND(right-arm-frozen): TEMPORARY. One `dq = 0` equality row per
        # configured joint, built exactly like the head pin above but over arbitrary model
        # joints. Names are validated so a typo cannot silently leave a right-arm joint free.
        # The q indices and nominal values let reset()/measured updates restore the pins to
        # nominal before zero velocity holds them there.
        self._dead_joints = list(cfg.dead_joints)
        unknown_dead = [n for n in self._dead_joints if n not in self._idx_q]
        if unknown_dead:
            raise ValueError(
                f"dead_joints {unknown_dead} are not joints of the reduced model "
                f"(valid: {sorted(self._idx_q)})"
            )
        self._dead_idx_q = [self._idx_q[n] for n in self._dead_joints]
        nominal = self.nominal_q()
        self._dead_nominal_q = nominal[self._dead_idx_q].copy()
        self._dead_joint_pin_A = np.zeros((len(self._dead_joints), self.model.nv))
        for row, name in enumerate(self._dead_joints):
            self._dead_joint_pin_A[row, self.model.idx_vs[self.model.getJointId(name)]] = 1.0
    ...
    def _frame_rigid_to_base(self, frame: str) -> bool:
        """True when every joint between the planar root and ``frame`` is a dead joint.

        Such a frame moves only with the chassis, so a FrameTask on it can no longer
        pose the link -- it can only drag the base (see ``_build_tasks_and_limits``).
        ``dead_joints`` is the only pin that can sit on an arm chain; the head pins are
        handled separately by ``_head_j1_pin_A``.
        """
        joint_id = self.model.frames[self.model.getFrameId(frame)].parentJoint
        dead = set(self._dead_joints)
        return all(
            self.model.names[j] in dead
            for j in self.model.supports[joint_id]
            if self.model.names[j] not in ("universe", "root_joint")
        )
    ...
    def _build_tasks_and_limits(self) -> None:
        ...
        # HW-WORKAROUND(right-arm-frozen): once dead_joints pins the right arm AND the
        # torso, every joint between the planar root and R_ee is fixed, so the gripper is
        # rigid to the chassis -- exactly like the fully-pinned head camera. The task can
        # no longer pose the gripper at all; the only thing it can still do is drag the
        # BASE, where it fights the equal-weight (10000) head world-position term for the
        # single free fore/aft DOF. That fight has no solution: the operator's
        # head->right-hand offset (~0.31 m fore) and the frozen arm's camera->R_ee offset
        # (~0.46 m fore) differ by a constant ~0.14 m that no amount of driving can close,
        # so the QP parks the base at the least-squares tie point. Measured on
        # books_08_24/episode_0: head error held +7.24 cm and R_ee error -7.24 cm, their
        # sum 0.00 +- 0.26 cm across all 476 frames -- the base handed the operator exactly
        # HALF of every forward step and never closed the standing deficit. Drop the task
        # while it is inert; the right arm still holds nominal through the dq = 0 pins, and
        # result.right_ee_error keeps reporting the (now honest) residual for the HUD.
        self.right_ee_task_active = not self._frame_rigid_to_base(RIGHT_EE_FRAME)
        self.tasks = [self.left_ee_task]
        if self.right_ee_task_active:
            self.tasks.append(self.right_ee_task)
        self.tasks += [self.base_task, self.posture_task]
    ...
    def reset(self, q: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        *not* anchored: :meth:`solve` re-sets it to the current base pose each step
        ...
        # HW-WORKAROUND(right-arm-frozen): configured pins are nominal-position locks,
        # not "hold wherever reset found them" locks. This keeps all right-arm FK and
        # emitted joint commands nominal even when a caller supplies a custom reset pose.
        if self._dead_idx_q:
            q0[self._dead_idx_q] = self._dead_nominal_q
    ...
    def solve(
        ...
        try:
            ...
            # HW-WORKAROUND(right-arm-frozen): BOTH EE tasks above remain active. These
            # equalities remove the right arm (and other configured pins) as QP levers, so
            # the right target is resolved only through whole-body DOFs that hardware can
            # execute while the left arm keeps all seven of its joints.
            self._add_dead_joint_equalities(problem)
        ...
        # HW-WORKAROUND(right-arm-frozen): Must land BEFORE integrate_inplace below -- this
        # makes the configured nominal-position locks exact rather than solver-tolerance
        # dependent for the whole session (see _apply_dead_joint_mask).
        velocity = self._apply_dead_joint_mask(velocity)
    ...
    def _update_from_measured(self, current_q: np.ndarray) -> None:
        ...
        q = q.copy()
        q[2:4] /= norm
        # HW-WORKAROUND(right-arm-frozen): measured feedback may report servo error, but
        # these DOFs are command/model invariants at nominal. Hardware startup homing gates
        # the actual error before engage; do not re-anchor a nominal lock to that error.
        if self._dead_idx_q:
            q[self._dead_idx_q] = self._dead_nominal_q
    ...
    def _add_dead_joint_equalities(self, problem: qpsolvers.Problem) -> None:
        """HW-WORKAROUND(right-arm-frozen): Stack ``dq = 0`` rows for configured pins.

        Same ``problem.A`` slot and same shape as the head pin / base lock rows. The other
        equality blocks touch only head DOFs, the planar root, and (via the torso-top row)
        the torso Jacobian. When torso joints are pinned too (see CAMERA-STABILITY in
        wbik_tmp.yaml), the torso-top row is fully covered and would degenerate to an
        all-zero equality, which is why that config disables the anchor. Both EE task rows
        remain in the objective. No-op when ``cfg.dead_joints`` is empty.
        """
        if self._dead_joint_pin_A.shape[0] == 0:
            return
        problem.A = np.vstack([problem.A, self._dead_joint_pin_A])
        problem.b = np.hstack([problem.b, np.zeros(self._dead_joint_pin_A.shape[0])])

    def _apply_dead_joint_mask(self, velocity: np.ndarray) -> np.ndarray:
        """HW-WORKAROUND(right-arm-frozen): Hard-zero pinned-joint velocity post-solve.

        The equality row above already drives these to zero, but only to the solver's
        tolerance. Make the nominal-pose invariant exact: a residual would otherwise
        integrate at 100 Hz for the whole session. Mirrors ``_apply_base_dof_mask``.
        """
        if not self._dead_idx_q:
            return velocity
        out = np.asarray(velocity, dtype=float).copy()
        for name in self._dead_joints:
            out[self.model.idx_vs[self.model.getJointId(name)]] = 0.0
        return out
