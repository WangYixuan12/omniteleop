...
def build_legs(args: Args) -> list[Leg]:
    ...
    return [
        Leg(
            f"turn {360 * abs(args.turn_revolutions):.0f}deg", 0.0, 0.0, args.turn_speed, turn_time
        ),
    ]
...
def _log_rate_probe(report: dict) -> None:
    ...
    if report.get("drive_recv"):
        r = report["drive_recv"]
        logger.info(
            f"[probe] drive receive cadence ~{r['hz_median']:.0f} Hz "
...
def main() -> None:
    ...
    logger.info(
        f"Zenoh config: {cfg if cfg else '<dexcomm default>'}; "
    ...
    _ = _wait(
        lambda: robot.chassis.steering_angle
        if np.asarray(robot.chassis.steering_angle).shape == (2,)
        else None,
        "chassis state",
    )
    ...
    if args.probe_rate:
        logger.info(
            f"Probing idle chassis state publish rates for {args.probe_duration_s:.0f}s "
    ...
    logger.info(
        f"Plan: {len(legs)} legs, ~{total_drive:.0f}s drive"
    ...
    for l in legs:
        ...
        logger.info(
            f"  {l.label:12s} cmd=({l.vx:+.2f},{l.vy:+.2f},{l.wz:+.2f}) "
    ...
    def recorder() -> None:
        ...
        while not rec_stop.is_set():
            ...
            rec["cmd_vx"].append(cmd[0])
            rec["cmd_vy"].append(cmd[1])
            rec["cmd_wz"].append(cmd[2])
            rec["cmd_label"].append(cmd_label)
            rec["steer_l"].append(steer[0])
            rec["steer_r"].append(steer[1])
            rec["wvel_l"].append(wvel[0])
            rec["wvel_r"].append(wvel[1])
            rec["enc_l"].append(enc[0])
            rec["enc_r"].append(enc[1])
            ...
            rec["odom_x"].append(odom_pose[0])
            rec["odom_y"].append(odom_pose[1])
            rec["odom_yaw"].append(odom_pose[2])
            rec["odom_vx"].append(odom_twist[0])
            rec["odom_vy"].append(odom_twist[1])
            rec["odom_w"].append(odom_twist[2])
            rec["cl_ref_x"].append(cl_ref[0])
            rec["cl_ref_y"].append(cl_ref[1])
            rec["cl_ref_yaw"].append(cl_ref[2])
            rec["cl_feedback_x"].append(cl_feedback[0])
            rec["cl_feedback_y"].append(cl_feedback[1])
            rec["cl_feedback_yaw"].append(cl_feedback[2])
            rec["cl_err_x"].append(cl_err[0])
            rec["cl_err_y"].append(cl_err[1])
            rec["cl_err_yaw"].append(cl_err[2])
            for i, v in enumerate(head):
                rec[f"head_j{i+1}"].append(float(v))
            for i, v in enumerate(torso):
                rec[f"torso_j{i+1}"].append(float(v))
            if args.record_images:
                rmsg = rgb_sub.get_latest()
                dmsg = depth_sub.get_latest()
    ...
    max_lin_speed, max_ang_speed = base_cl.closed_loop_limits(
        cl_config, args.speed, args.turn_speed
    )
    ...
    def set_cmd(
        ...
        with shared.lock:
            ...
            shared.cl_ref_pose = (
                nan3.copy() if ref_pose is None else np.asarray(ref_pose, dtype=np.float64).copy()
            )
            shared.cl_feedback_pose = (
                nan3.copy()
                if feedback_pose is None
                else np.asarray(feedback_pose, dtype=np.float64).copy()
            )
            shared.cl_pose_error = (
                nan3.copy()
                if pose_error is None
                else np.asarray(pose_error, dtype=np.float64).copy()
            )
    ...
    def run_open_loop_leg(leg: Leg) -> None:
        ...
        if not args.dry_run:
            if leg.wz != 0.0:
                robot.chassis.turn(
                    angular_speed=leg.wz, wait_time=leg.drive_time, center="base_center"
                )
            elif leg.vx != 0.0:
    ...
    def run_closed_loop_leg(leg: Leg) -> None:
        ...
        final_pose = base_cl.reference_pose(
            start_pose, reference_twist, leg.drive_time, leg.drive_time
        )
        ...
        while True:
            ...
            feedforward = (
                reference_twist if elapsed < leg.drive_time else np.zeros(3, dtype=np.float64)
            )
    ...
    achieved_cl_hz = (
        (cl_stats["iters"] / cl_stats["secs"]) if cl_stats["secs"] > 0 else float("nan")
    )
    if closed_loop:
        logger.info(
    ...
    if probe_report and probe_report.get("drive"):
        ...
        verdict = (
            "at publish ceiling"
            if achieved_odom_hz >= 0.9 * pub
            else "below ceiling (poll/CPU-bound)"
        )
    ...
    _write_hdf5(
        ...
        extra_attrs=dict(
            ...
            idle_publish_hz=(
                float(probe_report["drive"]["hz_median"])
                if (probe_report and probe_report.get("drive"))
                else float("nan")
            ),
        ),
...
def _write_hdf5(
    out: pathlib.Path, rec: dict, args: Args, legs: list[Leg], extra_attrs: Optional[dict] = None
) -> None:
    import h5py
    ...
    with h5py.File(out, "w") as f:
        f.attrs.update(
            dict(
                ...
                odom_rate=args.odom_rate,
                speed=args.speed,
                leg_time=args.leg_time,
                turn_speed=args.turn_speed,
                turn_revolutions=args.turn_revolutions,
                settle=args.settle,
                drive_state_mode=args.drive_state_mode,
        ...
        ag.create_dataset(
            "cmd_vel", data=np.column_stack([arr("cmd_vx"), arr("cmd_vy"), arr("cmd_wz")])
        )
        ...
        og.create_dataset(
            "pose", data=np.column_stack([arr("odom_x"), arr("odom_y"), arr("odom_yaw")])
        )
        og.create_dataset(
            "twist", data=np.column_stack([arr("odom_vx"), arr("odom_vy"), arr("odom_w")])
        )

        clg = f.create_group("control/closed_loop")
        clg.create_dataset(
            "reference_pose",
            data=np.column_stack([arr("cl_ref_x"), arr("cl_ref_y"), arr("cl_ref_yaw")]),
        )
        clg.create_dataset(
            "feedback_pose",
            data=np.column_stack(
                [arr("cl_feedback_x"), arr("cl_feedback_y"), arr("cl_feedback_yaw")]
            ),
        )
        clg.create_dataset(
            "pose_error_body",
            data=np.column_stack([arr("cl_err_x"), arr("cl_err_y"), arr("cl_err_yaw")]),
        )
        clg.attrs["source"] = cl_config.source

        jg = f.create_group("obs/joint")
        jg.create_dataset(
            "head", data=np.column_stack([arr("head_j1"), arr("head_j2"), arr("head_j3")])
        )
        jg.create_dataset(
            "torso", data=np.column_stack([arr("torso_j1"), arr("torso_j2"), arr("torso_j3")])
        )
        ...
        if args.record_images:
            ...
            ig.create_dataset(
                "head_left_rgb",
                data=_stack(rec["head_rgb"]).astype(np.uint8),
                compression="gzip",
                compression_opts=4,
            )
            ig.create_dataset(
                "head_depth",
                data=_stack(rec["head_depth"]).astype(np.float32),
                compression="gzip",
                compression_opts=4,
            )
        ...
        lg.create_dataset(
            "completion_reason", data=np.array([l.completion_reason for l in legs], dtype="S64")
        )
        lg.create_dataset(
            "abort_reason", data=np.array([l.abort_reason for l in legs], dtype="S256")
        )
...
def compute_metrics(rec: dict, legs: list[Leg], args: Args) -> dict:
    ...
    for leg in legs:
        ...
        drive_onset = (
            float(tw[sig > 0.1 * target][0] - leg.t_issue)
            if np.any(sig > 0.1 * target)
            else float("nan")
        )
        s_ref = pose_at(steer, leg.t_issue)
        steer_dev = np.abs(steer[win] - s_ref).max(axis=1) if win.any() else np.array([])
        steer_onset = (
            float(tw[steer_dev > 0.05][0] - leg.t_issue)
            if np.any(steer_dev > 0.05)
            else float("nan")
        )
        legs_out.append(
            dict(
                ...
                odom=dict(
                    dx=float(odom_d[0]),
                    dy=float(odom_d[1]),
                    dyaw_deg=float(np.degrees(odom_d[2])),
                    lin=float(np.hypot(odom_d[0], odom_d[1])),
                ),
                lin_err_odom_m=float(np.hypot(odom_d[0], odom_d[1]) - exp_lin)
                if not is_turn
                else None,
    ...
    cmd_speed = np.hypot(np.asarray(rec["cmd_vx"]), np.asarray(rec["cmd_vy"])) + np.abs(
        np.asarray(rec["cmd_wz"])
    )
    lag = _xcorr_lag(
        cmd_speed, meas_speed + meas_w, dt=float(np.median(np.diff(t))) if len(t) > 1 else 0.1
    )

    steer_lat = [
        l["latency_steer_onset_s"] for l in legs_out if np.isfinite(l["latency_steer_onset_s"])
    ]
    drive_lat = [
        l["latency_drive_onset_s"] for l in legs_out if np.isfinite(l["latency_drive_onset_s"])
    ]
    return dict(
        n_frames=len(t),
...
def _xcorr_lag(cmd: np.ndarray, meas: np.ndarray, dt: float, max_lag_s: float = 2.0) -> float:
    ...
    for k in range(max_lag + 1):  # only positive lags: response trails command
...
def _log_metrics(m: dict, legs: list[Leg]) -> None:
    logger.info(
        f"=== accuracy / latency  (rate {m['effective_rate_hz']:.1f} Hz, "
        f"{m['n_frames']} frames) ==="
    )
    for L in m["legs"]:
        o = L["odom"]
        extra = (
            f"yawErr(odom)={L['yaw_err_odom_deg']:+.1f}deg"
    ...
    c = m["closure"]
    logger.info(
        f"  return-to-origin: odom |xy|={1000 * c['odom_xy_m']:.0f}mm "
        f"yaw={c['odom_yaw_deg']:+.1f}deg"
    )
    lat = m["latency"]
    logger.info(
        f"  latency: xcorr={lat['xcorr_lag_s']:.2f}s  "
