...
- **`run_episode.py`** — `mobile=getattr(task,"MOBILE",False)`; `VegaOGEnv(..., lock_base=not mobile,
  mobile=mobile, ...)`; sets `env.base_x_max` for mobile. Renders head + 3rd-person mp4.
...
## Pipeline
```bash
(behavior) python sim_eval/collect_demos.py --task mobilepickplace --episodes 12 --out ~/Dexmate/data/sim_mobilepickplace
...
(dexmate_lerobot) python scripts/vis_episode_processed_wbc.py \
  --dataset_dir ~/Dexmate/data/sim_mobilepickplace/processed_wbc/train/sim_mobilepickplace \
  --episode_index 0
