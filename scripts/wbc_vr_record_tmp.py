...
HW-WORKAROUND(right-j1-dead) -- TEMPORARY FORK of ``wbc_vr_record.py``: the SIM counterpart
of ``wbc_vr_robot_tmp.py``, for validating the pinned-arm configuration before it touches
hardware. Delete with the rest of the ``*_tmp`` set once the R_arm_j1 motor is repaired.
Differences from canonical: imports ``whole_body_ik_tmp`` (which reads ``wbik_tmp.yaml`` ->
``dead_joints`` + the raised ``posture_cost_torso``), loads ``vr_teleop:`` from that same
file, prints a workaround banner, and stamps both values into ``/debug``. The leader
(``wbc_vr_leader.py``) is unchanged.

To A/B the torso-cost fix deterministically, do NOT try to repeat a hand motion twice --
record once, then replay the SAME stream through each setting (kinematic, no ``--physics``;
see the note on ``--physics`` non-determinism below)::

    # 1. record one take (whatever posture_cost_torso is in wbik_tmp.yaml)
    python scripts/wbc_vr_record_tmp.py --hdf5 /tmp/pin_take.hdf5
    # 2. set posture_cost_torso: 200 in wbik_tmp.yaml, replay
    python scripts/wbc_vr_record_tmp.py --replay /tmp/pin_take.hdf5 \
        --hdf5 /tmp/pin_c200.hdf5 --output /tmp/pin_c200.mp4 --no-viewer
    # 3. set it back to 5000, replay the SAME stream
    python scripts/wbc_vr_record_tmp.py --replay /tmp/pin_take.hdf5 \
        --hdf5 /tmp/pin_c5000.hdf5 --output /tmp/pin_c5000.mp4 --no-viewer

Then compare ``/debug/ik/torso`` between the two files: the same operator input drives both,
so any difference is the cost change alone.
...
from omniteleop.follower.whole_body_ik import (  # HW-WORKAROUND(right-j1-dead): sim-vs-solver guard
    WBCConfig as _CanonWBCConfig,
)
from omniteleop.follower.whole_body_ik_tmp import (  # HW-WORKAROUND(right-j1-dead)
...
# HW-WORKAROUND(right-j1-dead): read the vr_teleop: block from wbik_tmp.yaml, matching
# wbc_vr_robot_tmp.py, so sim and hardware shape the command stream from the SAME file and
# a sim-verified take replays identically on the robot.
...
def main() -> None:
    ...
    cfg = WBCConfig()
    # HW-WORKAROUND(right-j1-dead): the SAPIEN backends (wbc_sapien_sim, wbc_sapien_physics)
    # import DEFAULT_URDF / DEFAULT_NOMINAL_POSTURE / the joint-name lists from the CANONICAL
    # whole_body_ik, not from this fork -- so they read wbik.yaml while the solver reads
    # wbik_tmp.yaml. Those keys are identical today (the fork changes only dead_joints and
    # posture_cost_torso), but if they ever diverge the sim would silently model a different
    # robot than it solves for, which would invalidate exactly the sim-before-hardware check
    # this script exists for. Fail loudly instead.
    _canon_cfg = _CanonWBCConfig()
    _diverged = [
        name for name, tmp_val, canon_val in (
            ("urdf_path", cfg.urdf_path, _canon_cfg.urdf_path),
            ("nominal_posture", cfg.nominal_posture, _canon_cfg.nominal_posture),
        )
        if tmp_val != canon_val
    ]
    if _diverged:
        raise SystemExit(
            f"[wbc_vr] {WBIK_TMP_CONFIG_PATH.name} and wbik.yaml disagree on {_diverged}, "
            "but the SAPIEN backends load those from wbik.yaml while the solver loads "
            f"{WBIK_TMP_CONFIG_PATH.name} -- sim would not match the solver. Re-sync them."
        )
    ...
    # HW-WORKAROUND(right-j1-dead): announce the degraded config so a sim take is never
    # mistaken for a healthy-hardware one. No dead-joint SEEDING here (unlike the hardware
    # follower): sim has no readback, and ik.reset() already places both pinned joints at
    # nominal_posture -- which is where the real ones sit anyway (dead R_arm_j1 rests 0.24
    # deg from nominal; healthy L_arm_j1 is driven there by homing). So the pin alone
    # reproduces the hardware kinematics.
    if cfg.dead_joints:
        print(f"[wbc_vr] TEMPORARY WORKAROUND ACTIVE (config {WBIK_TMP_CONFIG_PATH.name}): "
              f"joints pinned out of the QP = {list(cfg.dead_joints)}; "
              f"posture_cost_torso={cfg.posture_cost_torso:g}. Affected arms solve with "
              "reduced DOF -- EE tracking is coarser and reachable workspace is smaller.")
    if not ik.right_ee_task_active:
        print("[wbc_vr] R_ee task DROPPED from the QP: its whole chain to the base is "
              "pinned, so it could only anchor the base against head tracking.")
    ik.reset()
    ...
    hdf5_meta = dict(
        ...
        # HW-WORKAROUND(right-j1-dead): stamp the degraded config into /debug so an A/B
        # between torso costs is self-identifying offline.
        dead_joints=",".join(cfg.dead_joints),
        posture_cost_torso=float(cfg.posture_cost_torso),
