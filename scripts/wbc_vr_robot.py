...
class HardwareDriver:
    ...
    # A workstation clock that is still being disciplined moves between takes. Measured
    # 2026-09-03 on xarm-workstation (systemd-timesyncd against an internet server with
    # 224 ms jitter): the robot-minus-workstation offset went from -411 ms to -338 ms in
    # 25 min, i.e. the camera-age gate saw every camera ~200 ms "late" and aborted the
    # first take of the run because the offsets had been measured once at startup.
    # Anything beyond this many ms since the previous measurement is worth a warning:
    # it is far above the 2-3 ms the LAN round trip can explain.
    _CAMERA_CLOCK_DRIFT_WARN_MS = 20.0

    def _refresh_camera_clock_offsets(self) -> None:
        """Re-measure camera-host clock offsets at every episode start.

        ``_init_recording`` measures them once; the age gate and ``meta/camera_ntp`` then
        assume they hold for the whole run. They do not when the workstation clock is
        being slewed, so re-query at each take, store the fresh calibration in this
        episode's static metadata, and report how far the offsets moved.
        """
        static = getattr(self, "_record_static", None)
        offsets = getattr(self, "_camera_clock_offsets_ns", None)
        if static is None or offsets is None:
            return  # recording metadata was never initialised (not --record)
        camera_ntp = self._query_camera_clock_calibrations()
        fresh = {label: int(np.asarray(camera_ntp[label]["offset_ns"])) for label in offsets}
        drift_ms = {label: (fresh[label] - offsets[label]) / 1e6 for label in offsets}
        worst = max(abs(v) for v in drift_ms.values())
        detail = ", ".join(f"{k}={v:+.1f}ms" for k, v in drift_ms.items())
        if worst > self._CAMERA_CLOCK_DRIFT_WARN_MS:
            print(
                f"[wbc_vr_robot] WARNING: camera clock offsets moved {detail} since the "
                "previous measurement. The workstation clock is being slewed (check "
                "`timedatectl timesync-status`; sync it to the robot over the LAN). "
                "Using the fresh offsets for this take; a long take can still drift past "
                "--record-max-camera-age-ms."
            )
        else:
            print(f"[wbc_vr_robot] camera clock offsets refreshed ({detail})")
        self._camera_clock_offsets_ns = fresh
        static["meta"]["camera_ntp"] = camera_ntp
        self._episode.set_static(static)
